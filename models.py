...
class Model(object):
    ...
    def save(self, sess=None):
        ...
        save_path = saver.save(sess,
                               "../data_tgcn/20ng/build/{}_best_models/%s.ckpt".format(FLAGS.dataset) % self.name)
    ...
    def load(self, sess=None):
        if not sess:
            raise AttributeError("TensorFlow session not provided.")
        saver = tf.train.Saver(self.vars)
        save_path = "../data_tgcn/20ng/build/{}_best_models/%s.ckpt".format(FLAGS.dataset) % self.name
