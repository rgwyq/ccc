...
#命令行参数解析器，用于解析命令行中传入的参数并设置相应的变量。
...
#用于编码输入文本的函数。
# 它使用了一个名为tokenizer的分词器（tokenizer）来对文本进行处理，并返回编码后的输入张量。
...
# 将数据分成训练集、验证集和测试集，并创建相应的数据集和数据加载器。
...
#测试模型的函数test_step和创建评估器evaluator的代码段。
...
#训练循环中的回调函数，用于在每个epoch完成时记录训练结果并保存模型的检查点。
